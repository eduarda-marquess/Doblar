...
void exibirMenuPrincipal() {
    ...
    while (opcao != 5) {
        ...
        Clic_printBox(55, 16);
        Clic_move(2, 22); Clic_setFgColor(Clic_color_YELLOW); printf("DOBLAR GAME");
        Clic_move(4, 5); 
        if (logado) {
            Clic_setFgColor(Clic_color_GREEN); printf("LOGADO: %s", usuarioAtivo);
            if (strcmp(usuarioAtivo, "duda123") == 0) { Clic_move(5, 5); printf(">> VANTAGEM ATIVA <<"); }
        } else { Clic_setFgColor(Clic_color_RED); printf("MODO: Visitante"); }
        ...
        Clic_move(13, 10); Clic_setFgColor(Clic_color_CIAN); printf("Opcao: ");
